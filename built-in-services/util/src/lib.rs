...
use protocol::types::{DataMeta, Event, Hash, MethodMeta, Receipt, ServiceContext, ServiceMeta};
...
impl<SDK: ServiceSDK> UtilService<SDK> {
    ...
    fn keccak256(
        ...
        if data.is_err() {
            return ServiceResponse::<KeccakResponse>::from_error((107, "data not valid"));
        }
    ...
    fn verify(
        ...
        if data_sig.is_err() {
            return ServiceResponse::<SigVerifyResponse>::from_error((108, "signature not valid"));
        };

        let data_pk = hex::decode(payload.pub_key.as_string_trim0x());
        if data_pk.is_err() {
            return ServiceResponse::<SigVerifyResponse>::from_error((109, "public key not valid"));
        };
